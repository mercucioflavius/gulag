...
echo "$($PSQL "SELECT teams.name FROM
                (SELECT winner_id as id FROM games WHERE round = 'Eighth-Final' and year = 2014
                UNION
                SELECT opponent_id FROM games WHERE round = 'Eighth-Final' and year = 2014)
                AS new LEFT JOIN teams ON new.id = teams.team_id
                ORDER BY teams.name")"
